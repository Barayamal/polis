...
function createTestApp() {
  ...
  app.post("/inspect", (req, res) => {
    res.json({ body: req.body, cookie: req.cookies?.polis });
  });

  app.get("/compress", (_req, res) => {
    res.type("text/plain").send("Polis middleware migration ".repeat(200));
  });
...
describe("supported Express HTTP middleware", () => {
  test("parses standard and structured-suffix JSON", async () => {
    ...
    const payload = { conversation_id: "synthetic-test", xid: "opaque-123" };

    for (const contentType of [
      "application/json",
      "application/vnd.polis+json",
    ]) {
  ...
  test("preserves nested urlencoded fields and parses cookies", async () => {
    ...
    const response = await request(app)
      ...
      .set("Cookie", "polis=opaque-session")
      .send("participant[choice]=agree&participant[round]=1");

    expect(response.status).toBe(200);
    expect(response.body).toEqual({
      body: { participant: { choice: "agree", round: "1" } },
      cookie: "opaque-session",
    });
  });

  test("rejects unsupported multipart bodies without a temp-file parser", async () => {
    const app = createTestApp();
    const response = await request(app)
      .post("/inspect")
      .field("conversation_id", "synthetic-test");
  ...
  test("compresses eligible responses", async () => {
    ...
    expect(response.text).toContain("Polis middleware migration");
  });
});
