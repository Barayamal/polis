...
# Rebuild the exact Alpine 3.24 BusyBox 1.37.0-r31 source/configuration with
# the upstream CVE-2025-60876 fix. The resulting statically linked binary is
# copied into each final Alpine image so it replaces, rather than wraps, the
# vulnerable multi-call binary.
...
curl --fail --location --silent --show-error \
  ...
  "$busybox_url" \
  --output "$work_dir/busybox.tar.bz2"
printf '%s  %s\n' "$busybox_sha256" "$work_dir/busybox.tar.bz2" \
  | sha256sum -c -

curl --fail --location --silent --show-error \
  --retry 3 --connect-timeout 20 \
  "$aports_url" \
  --output "$work_dir/aports-busybox.tar.gz"
printf '%s  %s\n' "$aports_sha256" "$work_dir/aports-busybox.tar.gz" \
  | sha256sum -c -

mkdir -p "$work_dir/source" "$work_dir/aports"
tar -xjf "$work_dir/busybox.tar.bz2" \
  --strip-components=1 \
  -C "$work_dir/source"
tar -xzf "$work_dir/aports-busybox.tar.gz" \
  -C "$work_dir/aports"

recipe_dir="$(find "$work_dir/aports" -type d -path '*/main/busybox' -print -quit)"
test -n "$recipe_dir"
test -f "$recipe_dir/APKBUILD"
test -f "$recipe_dir/busyboxconfig"
...
printf '%s\n' "$patch_names" | while IFS= read -r patch_name; do
  test -f "$recipe_dir/$patch_name"
  patch -d "$work_dir/source" -p1 < "$recipe_dir/$patch_name"
done

patch -d "$work_dir/source" -p1 \
  < /opt/fncp-busybox-fix/CVE-2025-60876.patch
grep -Fq 'Unencoded control character found in the URL!' \
  "$work_dir/source/networking/wget.c"
...
sed \
  ...
  "$recipe_dir/busyboxconfig" \
  > "$work_dir/build-static/.config"
...
if /out/busybox wget "$bad_url" \
  >"$work_dir/wget.stdout" \
  2>"$work_dir/wget.stderr"; then
  echo "patched BusyBox accepted a control character in a URL" >&2
  exit 1
fi
grep -Fq 'Unencoded control character found in the URL!' \
